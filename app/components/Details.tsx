...
const ScoreBadge = ({ score }: { score: number }) => {
  ...
  return (
    ...
    >
      <img
        src={s > 69 ? "/icons/check.svg" : "/icons/warning.svg"}
        alt="score"
        className="size-4"
      />
      <p
...
export default function Details({ feedback }: { feedback: Feedback }) {
  const tone = feedback?.toneAndStyle;
      ...
      <Accordion>
        <AccordionItem id="tone-style">
          <AccordionHeader itemId="tone-style">
            <CategoryHeader
              title="Tone & Style"
              categoryScore={tone?.score ?? 0}
            />
          </AccordionHeader>
          <AccordionContent itemId="tone-style">
            <CategoryContent tips={(tone?.tips ?? []) as Tip[]} />
          </AccordionContent>
        </AccordionItem>

        <AccordionItem id="content">
          <AccordionHeader itemId="content">
            <CategoryHeader title="Content" categoryScore={content?.score ?? 0} />
          </AccordionHeader>
          <AccordionContent itemId="content">
            <CategoryContent tips={(content?.tips ?? []) as Tip[]} />
          </AccordionContent>
        </AccordionItem>
        ...
        <AccordionItem id="structure">
          ...
          <AccordionContent itemId="structure">
            <CategoryContent tips={(structure?.tips ?? []) as Tip[]} />
          </AccordionContent>
        </AccordionItem>

        <AccordionItem id="skills">
          <AccordionHeader itemId="skills">
            <CategoryHeader title="Skills" categoryScore={skills?.score ?? 0} />
          </AccordionHeader>
          <AccordionContent itemId="skills">
            <CategoryContent tips={(skills?.tips ?? []) as Tip[]} />
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </div>
